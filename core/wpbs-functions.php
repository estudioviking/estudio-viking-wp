<?php
/**
 * Functions to adjusting theme to Bootstrap
 *
 ...
 */
function wpbs_paginate_links( $args = '' ) {
 ...
 * 
 * @see wpbs_paginate_links()
 * @since 1.0.0
 * 
 * @param	string|array	$args	Optional args. See wpbs_paginate_links().
 * @return	string					Markup for pagination links.
 * ----------------------------------------------------------------------------
 */
function wpbs_pagination_links( $args = array() ) {
	...
	if ( 'posts' == $args['paginate_content'] ) {
		...
		if ( $GLOBALS['wp_query']->max_num_pages > 1 ) {
			...
			$links = wpbs_paginate_links( $args );
		}
	}
	...
	$output = '';
	$output = wpbs_navigation_markup( $links, $args );
 ...
 * ============================================================================
 */
function wpbs_navigation_markup( $links, $args = array() ) {
	if ( empty( $links ) ) return;
 ...
 * ============================================================================
 */
function wpbs_post_navigation( $args = array() ) {
	$args = wp_parse_args( $args, array(
	...
	if ( $prev || $next ) {
		$navigation = wpbs_navigation_markup( $prev . $next, $args );
	}
 ...
 * @since 1.0.0
 */
function wpbs_make_label_tags( $links ) {
...
add_filter( 'term_links-post_tag', 'wpbs_make_label_tags' );
 ...
 * @return	void
 */
function wpbs_comments_loop( $comment, $args, $depth ) {
 ...
 * @since 1.0.0
 */
function wpbs_make_btn_comment_reply_link( $link, $args, $comment, $post ) {
